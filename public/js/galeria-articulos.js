...
const articleImgContainer = document.querySelectorAll(".article-img-container"),
    ...
    galleryPicturesList = galleryArticles.querySelector(".modal-gallery__pictures-list");
...
function setGalleryPictures(imgJSON) {
    // Reseteo la galleria
...
function showGallery(state) {
    ...
    if (!state) galleryPicturesList.classList.remove("show-gallery__pictures-list");
    else {
...
articleImgContainer.forEach(function (img) {
    img.addEventListener("click", function () {
        ...
        const requestUrl = "/public/JSON/Mascotas/mascota1.json";
...
document.addEventListener("click", function (e) {
    ...
    //     }
    // }
    if (e.target.classList.contains("modal-close") || e.target.classList.contains("modal-close__icon")) {
        if (galleryArticles.classList.contains("show-modal-back")) {
            showGallery(0);
        }
    }
